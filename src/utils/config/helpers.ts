...
export function resolveLanguageDetectionConfigForModeChange(
  ...
  const availableIds = getProviderIdsForCapability(
    "languageDetection",
    providersConfig,
    { requireEnable: true },
  )
...
export function findFeatureMissingProvider(
  ...
  if (
    ...
    getProviderIdsForCapability("languageDetection", remainingProviders, { requireEnable: true }).length === 0
  ) {
...
export function computeLanguageDetectionFallbackAfterDeletion(
  ...
  return getProviderIdsForCapability("languageDetection", remainingProviders, { requireEnable: true })[0]
}
