...
export async function translateTextCore(options: TranslateTextOptions): Promise<string> {
  ...
  const normalizedWebPageContext = normalizeWebPagePromptContext(webPageContext)
  const providerRef = await serializeProviderRef(providerConfig)
...
export function validateTranslationConfigAndToast(
  ...
  if (!provider) {
    toastManager.add({ type: "error", title: i18n.t("options.apiProviders.addProvider") })
    return false
  }
  ...
  if (
    ...
    !["deeplx", "ollama", "openai-compatible", "open-responses"].includes(provider.config.provider)
  ) {
