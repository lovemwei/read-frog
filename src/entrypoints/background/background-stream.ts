...
const streamTextPayloadSchema = baseStreamPayloadSchema.extend({
...
export async function generateTextForProviderRef(
  ...
  if (providerRef.kind !== "local" || !providerRef.config) {
    throw new BackgroundStreamError("invalid_request", "A custom provider is required")
  }
...
export async function runStreamTextInBackground(
  ...
  const parsed = streamTextPayloadSchema.safeParse(serializablePayload)
  if (!parsed.success) throw new BackgroundStreamError("invalid_request", invalidStreamStartPayloadMessage)
...
export async function runStructuredObjectStreamInBackground(
  ...
  const partStream = await createLocalStructuredObjectPartStream(serializablePayload, objectSchema, options)
